...
def wLink(data, text, extra=""):
    return "<a href='" + data + "'" + extra + " target='_blank'>" + text + "</a>"
...
def wBicings(stationsS, stationsB, id):
    res = "<td>"
    res += wLink("#collapseS" + str(id), str(len(stationsS)) + " Estacions amb espai", "data-toggle='collapse'")
    res += "<div id='collapseS" + str(id) + "' class='panel-collapse collapse'><ul class='list-group'>"
    for s in stationsS:
        res += "<li class='list-group-item'>" + s.toHTML() + "</li>"
    res += "</ul></div><br>"

    res += wLink("#collapseB" + str(id), str(len(stationsB)) + " Estacions amb bicis", "data-toggle='collapse'")
...
<head>
    ...
    <style>table { table-layout: auto; white-space: nowrap; }</style>
</head>
